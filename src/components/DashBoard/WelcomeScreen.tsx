  ...
  return (
    <div className='bg-white rounded-lg border border-gray-100 p-6 flex flex-col lg:flex-row justify-between'>
        <div>
          ...
          {UserLoading ? <div className="h-7 w-24 bg-black/10 animate-pulse rounded lg:mt-1" /> : <h1 className='lg:text-2xl capitalize text-blue-500 lg:mt-1 font-semibold'>{user ? `${user.first_name} ${user.last_name}` : "Unknown"}</h1>}
          
          
          {UserLoading ? <div className="h-4 w-24 bg-black/10 animate-pulse rounded mt-1" /> : <h6 className='text-xs'>{user ? `${user.groups}` : "Unknown"}</h6>}
