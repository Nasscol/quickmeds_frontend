  ...
  return (
    <div className={`flex flex-row gap-x-6 justify-between rounded-xl ${bgColor ? bgColor : ""} w-max cursor-pointer hover:-translate-y-2 transition-all rounded-xl border bg-card p-6`}>
      ...
      <div className="flex flex-col">
        ...
        {isLoading || isFetching ? <div className="h-7 w-24 bg-black/10 animate-pulse rounded" /> : <p className={`xl:text-xl tracking-tight ${textColor ? textColor : "text-black"}`}>{Number(value).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 })} <span className="text-black">{isMoney && "shs"}</span></p>}
        {percentage_change != null && (isLoading || isFetching ? <div className="h-4 mt-2 w-34 bg-black/10 animate-pulse rounded" /> : <div className={`mt-2 text-xs xl:text-sm ${percentage_change >= 0 ? "text-green-600" : "text-red-600"}`}><p>{percentage_change >= 0 && "+"}{Number(percentage_change).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}<span>% from yesterday</span></p></div>)}
      </div>
